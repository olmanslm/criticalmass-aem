...
public class CreatePageIT {
     ...
     */
    @ClassRule
    public static final CQAuthorClassRule CQ_BASE_CLASS_RULE =
            new CQAuthorClassRule();
     ...
     * the same instance. By removing the page at the end of the test
     * execution, you are not going to leave any clutter on the instance under
     * test.
     */
    @Rule
    public Page root = new Page(CQ_BASE_CLASS_RULE.authorRule);
    ...
    public static void beforeClass() {
        adminAuthor = CQ_BASE_CLASS_RULE.authorRule
    ...
    public void testCreatePage() throws InterruptedException {
        // Assert that the page (created with the Page rule above) exists on
        // the admin instance. Under the hood, this assertion implements a
        ...
        CQAssert.assertCQPageExistsWithTimeout(adminAuthor, root.getPath(),
                TIMEOUT, HTTP_INTERNAL_SERVER_ERROR);
    }
